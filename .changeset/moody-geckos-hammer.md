...
feat: Resolve custom media queries inside `not` and compound `and`/`or` conditions
...
```css
/* input */
@media not (--breakpoint) { .a { color: red } }
@media (--breakpoint) and (color) { .a { color: red } }
```

```css
/* output */
@media (width > 100em) { .a { color: red } }
@media (width <= 100em) and (color) { .a { color: red } }
```
